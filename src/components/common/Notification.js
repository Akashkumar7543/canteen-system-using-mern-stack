...
const Notification = () => {
  ...
  const [hasNewNotifications, setHasNewNotifications] = useState(true); // Example: Initially true

  const toggleDropdown = () => {
    setIsOpen(!isOpen);
  };

  // Sample notifications array (replace with actual data from API or state)
  const notifications = [
    {
      id: 1,
