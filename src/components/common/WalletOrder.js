...
import { fetchAllWallteOrders } from "../../slices/ProfileeSlice";
...
const WalletOrder = () => {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      try {
        const recentOrders = await fetchAllWallteOrders();
        setOrders(recentOrders);
      } catch (error) {
        // setError("No Order Yet Order Now");
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="man">
     <div>
        <div className="wallte col-sm-6 col-md-8 mb-4 mr-4">
          <div className="containeeer22 mt- mr-9 ">
            ...
            {loading ? (
              ...
              <p className="no-payment-history">.</p>
            ) : (
              orders.map((order) => (
